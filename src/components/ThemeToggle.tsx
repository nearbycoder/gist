...
export const getThemeCookie = createServerFn().handler(() => {
  return themeModeSchema.catch('auto').parse(getCookie('theme') ?? 'auto');
});
...
export function ThemeToggle() {
  ...
  return (
    <div
      ...
      className={twMerge(
        `w-12 h-6 bg-zinc-500/10 dark:bg-zinc-800 rounded-full flex items-center justify-between cursor-pointer relative transition-all`
      )}
    >
